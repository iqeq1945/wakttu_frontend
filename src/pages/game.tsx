...
export const getServerSideProps: GetServerSideProps = async (context) => {
    try {
        ...
        if (!gameState || gameState.type === undefined || gameState.id === undefined || isNaN(id) || isNaN(type)) {
            return {
                notFound: true,
            };
        }
...
const fetchGameStateFromServer = async (id: number, type: number): Promise<GameStateProps | null> => {
...
const Game: React.FC<GamePageProps> = ({ gameState }) => {
    ...
    if (!gameState?.id || !gameState?.type || gameState.id === null || gameState.type === null) return null;
